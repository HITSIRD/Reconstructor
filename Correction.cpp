...
struct ReProjectionError {
    const Eigen::Matrix4d PV; // view space to screen space transform matrix, PV = M_viewport * M_per
    const Eigen::Vector4d point_3d;
    const double x;
    const double y;

    ReProjectionError(const float4x4 &M_per, const float4x4 &M_viewport, const float4 &_point_3d, double _x, double _y)
...
struct ReProjectionErrorEulerAngle {
    ...
    ReProjectionErrorEulerAngle(
            const float4x4 &M_per, const float4x4 &M_viewport, const float4 &_point_3d, double _x, double _y): PV(
            (M_viewport * M_per).cast<double>()), point_3d(_point_3d.cast<double>()), x(_x), y(_y) {}

    template<typename T> bool operator()(const T *const view, T *residual) const {
        Eigen::Matrix<T, 4, 4> M_view;
        Eigen::Matrix<T, 3, 3> R;
        R = Eigen::AngleAxis<T>(view[0], Eigen::Vector<T, 3>::UnitZ()) *
                Eigen::AngleAxis<T>(view[1], Eigen::Vector<T, 3>::UnitY()) *
                Eigen::AngleAxis<T>(view[2], Eigen::Vector<T, 3>::UnitX());

        M_view << R, Eigen::Vector<T, 3>(view[3], view[4], view[5]), Eigen::RowVector<T, 4>(T(0), T(0), T(0), T(1.0));
...
void Correction::correction(
        Uniform *uniform, const std::vector<pair<index_t, index_t>> &matches3D2D,
        const std::vector<pair<index_t, index_t>> &matches2D2D, Vertex *vertices, sfs::CORRECTION_MODE mode) {
    switch (mode) {
        case sfs::MATRIX:
            correctionMatrix(uniform, matches3D2D, matches2D2D, vertices);
            break;
        case sfs::EULER_ANGLE:
            correctionEuler(uniform, matches3D2D, matches2D2D, vertices);
            break;
    }
}

void Correction::correctionMatrix(
        Uniform *uniform, const std::vector<pair<index_t, index_t>> &matches3D2D,
        const std::vector<pair<index_t, index_t>> &matches2D2D, Vertex *vertices) {
    ...
    for (index_t i = 0; i < uniform->referImage->numCameras; i++) {
        ...
        const float4x4 &M_viewport = uniform->cameras->at(i)->matrixViewport;
        const float4x4 &M_per = uniform->cameras->at(i)->matrixPerspective;
        float4x4 &M_view = uniform->cameras->at(i)->matrixView;
        ...
        cout << view[0] << " " << view[1] << " " << view[2] << " " << view[3] << " " << view[4] << " " << view[5] << " "
                ...
                << endl;
        for (const auto &p1: matches3D2D) {
            index_t refer_index;
            index_t start = i * image_size;
            index_t end = start + image_size;
            for (const auto &p2: matches2D2D) {
                if (p2.second >= start && p2.second < end && p2.first == p1.second) {
                    ...
                    ceres::CostFunction *cost_func =
                            ReProjectionError::create(M_per, M_viewport, vertices[p1.first].position,
                                    (double)point_x + 0.5, (double)point_y + 0.5);
        ...
        // update
        uniform->cameras->at(i)->matrixWorldToScreen = M_viewport * M_per * M_view;
...
void Correction::correctionEuler(
        Uniform *uniform, const std::vector<pair<index_t, index_t>> &matches3D2D,
        const std::vector<pair<index_t, index_t>> &matches2D2D, Vertex *vertices) {
    index_t image_x = uniform->referImage->x;
    index_t image_y = uniform->referImage->y;
    ...
    for (index_t i = 0; i < uniform->referImage->numCameras; i++) {
        ...
        float3x3 R;
        R << M_view(0, 0), M_view(0, 1), M_view(0, 2), M_view(1, 0), M_view(1, 1), M_view(1, 2), M_view(2, 0), M_view(2,
                1), M_view(2, 2);
        ...
        for (const auto &p1: matches3D2D) {
            ...
            for (const auto &p2: matches2D2D) {
                if (p2.second >= start && p2.second < end && p2.first == p1.second) {
                    ...
                    //                    cout << point_x << " " << point_y << endl;
                    ceres::CostFunction *cost_func =
                            ReProjectionErrorEulerAngle::create(M_per, M_viewport, vertices[p1.first].position,
                                    (double)point_x + 0.5, (double)point_y + 0.5);
                    problem.AddResidualBlock(cost_func, nullptr, view);
                    break;
                }
